...
/// </summary>
/// <remarks>Kudos to // https://stackoverflow.com/a/4748886</remarks>
public sealed class ScriptEngine : IDisposable
{
    ...
    private const int ENotimpl = -2147467263;
    ...
    public static Version GetVersion(string language)
    {
        ...
        int major = GetProperty(scriptProperty, ScriptpropMajorversion, 0);
        int minor = GetProperty(scriptProperty, ScriptpropMinorversion, 0);
        int revision = GetProperty(scriptProperty, ScriptpropBuildnumber, 0);
    ...
    public ScriptEngine(string language)
    {
        ...
        if (IntPtr.Size == 4)
        {
            parse32 = (IActiveScriptParse32)this.engine;
            parse32.InitNew();
        }
        else
        {
            parse64 = (IActiveScriptParse64)this.engine;
            parse64.InitNew();
        }
    }

    private const int ScriptpropName = 0x00000000;
    private const int ScriptpropMajorversion = 0x00000001;
    private const int ScriptpropMinorversion = 0x00000002;
    private const int ScriptpropBuildnumber = 0x00000003;
    ...
    public Version Version
    {
        get
        {
            if (version == null)
            {
                int major = GetProperty(ScriptpropMajorversion, 0);
                int minor = GetProperty(ScriptpropMinorversion, 0);
                int revision = GetProperty(ScriptpropBuildnumber, 0);
                version = new Version(major, minor, Environment.OSVersion.Version.Build, revision);
            }
            return version;
        }
    }
    ...
    public string Name
    {
        get
        {
            if (name == null)
            {
                name = GetProperty(ScriptpropName, string.Empty);
            }
            return name;
        }
    }
    ...
    {
        value = null;
        IActiveScriptProperty property = engine as IActiveScriptProperty;
        if (property == null)
            return false;
    ...
    internal class ScriptSite : IActiveScriptSite
    {
        ...
        {
            item = IntPtr.Zero;
            if ((returnMask & ScriptInfo.TypeInfo) == ScriptInfo.TypeInfo)
                return ENotimpl;
    ...
    private object Parse(string text, bool expression)
    {
        ...
        try
        {
            ...
            if (parse32 != null)
            {
                ...
                parse32.ParseScriptText(text, null, IntPtr.Zero, null, 0, 0, flags, out result, out exceptionInfo);
            }
            else
            {
                parse64.ParseScriptText(text, null, IntPtr.Zero, null, 0, 0, flags, out result, out exceptionInfo);
            }
        }
        catch
        {
